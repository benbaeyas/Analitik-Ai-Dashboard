...
### Tokens — Data Visualization (Syntax Theme)

For charts, graphs, and stakeholder-facing visualizations, we borrow colors from code editor syntax highlighting. This provides a playful, distinct hierarchy for data that remains grounded in the technical theme.

 Name  Value  Token  Role 
--------------------------
 Viz Primary  `#ef6f2e`  `--color-viz-primary`  Main data points, active metrics, primary chart lines (Signal Orange) 
 Viz Secondary  `#d49228`  `--color-viz-secondary`  Secondary data series, comparisons, warnings (Syntax Yellow) 
 Viz Tertiary  `#368c9c`  `--color-viz-tertiary`  Tertiary data, complementary metrics (Terminal Cyan) 
 Viz Quaternary  `#4f8f52`  `--color-viz-quaternary`  Success metrics, growth trends, fourth data series (String Green) 
 Viz Quinary  `#8758a3`  `--color-viz-quinary`  Complex data categories, fifth series (Keyword Purple) 
 Viz Base  `#8a8380`  `--color-viz-base`  Inactive data, grid lines, baselines (Granite Mute) 
...
### CLI Install Box
Role Primary installCTA container

White (#fafafa) panel with 1px Pewter Rule border, 4px radius, 16px padding. Contains a tab row (MACOS  LINUX, WINDOWS) in Geist Mono 14px uppercase, then a command row orange #ef6f2 prompt chevron '', monospace command 'curl -fsSL httpsapp.factory.aicli  sh', #020202 text, and a small copy icon on the right. The orange chevron is the only chromatic element — a signal that this is a runnable command.
...
### News Card
Role Editorialupdates entry

Two cards per row, 4px radius, no border or 1px Pewter Rule. Top Category Badge (ENGINEERINGNEW). Heading at 24px Geist #020202, -0.72px tracking, single line. Body in 16px #5c5855, 2–3 lines truncated. 'LEARN MORE →' as ghost text link. Vertical timeline (●──●──●──●) on far right with 4px dots in #ef6f2 and #b8b3b0, connected by 1px dashed lines.

### Feature Section
Role Product capability block

Two-column alternating text-leftvisual-right then text-rightvisual-left. Left column max ~480px Version Dot Indicator, heading at 48px Geist #020202 -2.3px tracking, body 16px #5c5855, LEARN MORE → link. Right column IDE Panel Frame or Terminal Window. Vertical gap 80px between sections.
...
- Do not use weight variation (boldsemibold) for hierarchy — Geist ships only at 400; use size and tracking instead
...
-----------------------------
 ...
 2  Dotted Frame  `#d6d3d2`  Container stroke for IDEterminal mockups, hairline structural lines 
...
root {
  ...
  --color-ember-edge #d15010;
   ...
   Data Visualization 
  --color-viz-primary #ef6f2e;
  --color-viz-secondary #d49228;
  --color-viz-tertiary #368c9c;
  --color-viz-quaternary #4f8f52;
  --color-viz-quinary #8758a3;
  --color-viz-base #8a8380;
