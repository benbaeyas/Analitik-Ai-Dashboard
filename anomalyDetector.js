...
function detectIQROutliers(data) {
  ...
  return {
    ...
    pctOutliers:   (data.length > 0 ? (outliers.length / data.length * 100).toFixed(1) : 0),
    bySubcat:      bySubcat
  };
}
...
function detectAllAnomalies(data) {
  ...
  anomalies.severityCount = countSeverity(anomalies);
...
// narrateAllAlerts() has been moved into getUnifiedAIResponse() in aiInsight.js
// for token-efficient single-call AI processing.
